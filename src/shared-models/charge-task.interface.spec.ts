...
import {
  ...
  relDate,
  dateFromRelative,
} from './charge-task-wire.interface'
...
describe('FrontEnd Interface', () => {
  it('1', () => {
    ...
    expect(dateFromRelative(1, '10', '30', now).toLocaleString()).toBe('19/10/2023 10:30:00')
  })
  it('2', () => {
    const now = new Date(2023, 9, 18, 0, 0, 0)
    expect(dateFromRelative(0, '03', '00', now).toLocaleString()).toBe('18/10/2023 03:00:00')
  })
  it('3', () => {
    const now = new Date(2023, 9, 18, 23, 59, 0)
    expect(dateFromRelative(1, '03', '00', now).toLocaleString()).toBe('19/10/2023 03:00:00')
  })
  it('3', () => {
    const now = new Date(2023, 9, 18, 23, 59, 0)
    expect(dateFromRelative(0, '01', '00', now).toLocaleString()).toBe('18/10/2023 01:00:00')
  })
})
